...
1. **Time-series data:** 
      Use a vertical chart to display changes over time, such as sales figures per quarter or website visits per month. 

2. **Ordinal data:** 
      Choose this chart for categories that have a logical order, like age groups, educational levels, or performance ratings. The categories can be arranged along the x-axis in their natural sequence. 

3. **Few categories:** 
      Vertical charts are best for a limited number of categories to keep the x-axis from becoming too crowded. 
...
**Color highlighting serves following purpose:**
      
1. **Focus:** 
      Directs attention to Colorado (COL) instantly via pre-attentive color, creating visual hierarchy.
      
2. **Context:** 
      Keeps all other stadiums visible in grey for fair comparison while emphasizing the focal category.
      
3. **Story:** 
      Visually reinforces the narrative (“Is Coors Field most run-friendly?”) without changing quantitative encoding.
      
4. **Clarity:** 
      Avoids label scanning; reduces cognitive load; makes the takeaway skimmable and memorable.


**Position (x/y) vs color:**
      - Position encodes quantitative values with highest precision and is easiest to compare across items; use it for the primary metric (bar length/axis location).
      - Color is categorical/ordinal emphasis with lower quantitative precision; use it to highlight groups or a focal category (e.g., COL) without replacing the primary encoding.
...
# Add annotation for Colorado with improved placement
...
ax.annotate(f"COL: {colorado_runs:.2f} runs/game", 
            ...
            xytext=(colorado_runs + 1.5, colorado_index + 0.3),
            arrowprops=dict(arrowstyle='->', color='darkorchid', lw=2, connectionstyle='arc3,rad=0.1'),
            fontsize=11, fontweight='bold', color='darkorchid',
            bbox=dict(boxstyle='round,pad=0.5', facecolor='white', edgecolor='darkorchid', alpha=0.8))

# Set x-axis to start from 0 and extend enough to accommodate annotation
max_runs = max(avgDF_2010_sorted.totalRuns)
ax.set_xlim(0, max(colorado_runs + 3, max_runs * 1.15))

# Control tick label font sizes (the numbers/values on axes)
ax.tick_params(axis='x', labelsize=10)  # x-axis tick numbers
ax.tick_params(axis='y', labelsize=9)   # y-axis stadium names
...
1. **Clear, Bold Title**: The title immediately states the conclusion ("Colorado is the Most Run-Friendly Ballpark"), making the message clear to executives who might only glance at the chart.

2. **Professional Styling**: 
      - Seaborn whitegrid style provides a clean, modern appearance
      - Consistent font sizes (16 for title, 12 for labels) create hierarchy
      - Subtle grid lines (alpha=0.3) aid reading without cluttering

3. **Strategic Color Use**:
      - Dark purple for Colorado creates immediate visual emphasis
      - Light grey for other stadiums provides context without distraction
      - Legend explains the color coding clearly

4. **Proper Axis Formatting**:
      - Descriptive axis labels ("Average Runs Per Game" is clear and unambiguous)
      - X-axis starts at zero (critical for accurate comparisons)
      - X-axis extends 10% beyond max value to accommodate annotation

5. **Visual Emphasis on Key Finding**:
      - Colorado highlighted with color
      - Sorted data makes ranking immediately apparent
      - Annotation provides exact numerical value

6. **Complete Information**:
      - All stadiums shown (no data hidden)
      - Summary statistics provided below for detailed analysis
      - Legend clarifies the color scheme

7. **Appropriate Size**: figsize=(8, 6) is suitable for presentations - readable without dominating the slide

**Is the annotation helpful? Can you fix its placement?**

- **Yes, the annotation is helpful because it**:
      - Provides the exact numerical value (10.59 runs/game) without requiring the viewer to estimate from the axis.
      - Reinforces the visual message with text.
      - Creates a clear callout for Colorado's performance.

- **However, the current placement has issues**:
      - The text is positioned too close to the bar end (only 0.5 units away), which can cause overlap.
      - The arrow and text may crowd the visualization.
      - Better placement would position the text further to the right or use a different annotation style.

Annotation placement fixed in the above code to reflect better visualization.
...
# Control tick label font sizes (the numbers/values on axes)
axes[0].tick_params(axis='x', labelsize=9)  # x-axis tick numbers
axes[0].tick_params(axis='y', labelsize=8)  # y-axis stadium names (smaller for readability)
...
axes[0].annotate(f"COL: {col_2010_runs:.2f} runs/game", 
                ...
                xytext=(col_2010_runs + 1.5, col_2010_idx + 0.3),
                arrowprops=dict(arrowstyle='->', color='darkorchid', lw=2, connectionstyle='arc3,rad=0.1'),fontsize=5, fontweight='bold', color='darkorchid',
                bbox=dict(boxstyle='round,pad=0.5', facecolor='white', edgecolor='darkorchid', alpha=0.8))

# 2021 visualization
bars_2021 = axes[1].barh(comparison_data_2021.home, comparison_data_2021.runs_2021, color=colors_2021)
axes[1].set_title("2021 Season: Average Runs Per Game by Stadium", fontsize=10, fontweight='bold')
axes[1].set_xlabel("Average Runs Per Game", fontsize=10)
axes[1].set_ylabel("Stadium (Home Team)", fontsize=10)
# Control tick label font sizes (the numbers/values on axes)
axes[1].tick_params(axis='x', labelsize=9)  # x-axis tick numbers
axes[1].tick_params(axis='y', labelsize=8)  # y-axis stadium names (smaller for readability)
...
axes[1].annotate(f"COL: {col_2021_runs:.2f} runs/game", 
                ...
                xytext=(col_2021_runs + 1.5, col_2021_idx + 0.3),
                arrowprops=dict(arrowstyle='->', color='darkorchid', lw=2, connectionstyle='arc3,rad=0.1'),fontsize=5, fontweight='bold', color='darkorchid',
                bbox=dict(boxstyle='round,pad=0.5', facecolor='white', edgecolor='darkorchid', alpha=0.8))
